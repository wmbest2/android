...
func DecompressXML(xml []byte) string {
...
// Compressed XML file/bytes starts with 24x bytes of data,
    ...
    xmlTagOff := LEW(xml, 3*4)  // Start from the offset in the 3rd word.
    // Scan forward until we find the bytes: 0x02011000(x00100102 in normal int)
    for ii := xmlTagOff; ii < len(xml)-4; ii += 4 {
    ...
    for off < len(xml) {
        ...
        if (tag0 == startTag) { // XML START TAG
            numbAttrs := LEW(xml, off+7*4)  // Number of Attributes to follow
            ...
            for ii := 0; ii < numbAttrs; ii++ {
                attrNameSi := LEW(xml, off+1*4)  // AttrName String Index
                attrValueSi := LEW(xml, off+2*4) // AttrValue Str Ind, or FFFFFFFF
                attrResId := LEW(xml, off+4*4)  // AttrValue ResourceId or dup AttrValue StrInd
